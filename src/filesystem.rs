...
pub unsafe fn inject_hooks() {
    unsafe {
        ...
        let get_directory_path_params_hk_addr =
            get_directory_path_parameters as *const () as Address;
    ...
    unsafe {
        ...
        if bytes_written != 0 {
            ...
            let mut current_directory_str =
                std::str::from_utf8_unchecked(&directory_buffer[..bytes_written as usize])
                    .to_string();
...
unsafe fn find_file_impl(a1: *const u8) -> u32 {
    unsafe {
        ...
        *(0x4F52B0 as *mut *const u8) = a1;
        ...
        match FindFirstFileA(lp_filename, find_file_data) {
            Ok(find_first_file_result) => {
...
pub unsafe fn close_file(file_handle: HANDLE) -> u32 {
    unsafe {
        *(FILE_HANDLE as *mut HANDLE) = HANDLE::default();
...
pub unsafe fn open_or_create_file(
    ...
    unsafe {
        ...
        write_memory(FILE_HANDLE, &current_file_pattern);
...
pub unsafe fn set_file_pointer_hooked(
    ...
    unsafe { SetFilePointer(file_handle, distance_to_move, None, dw_move_method) }
}
...
unsafe fn load_file(
    ...
    unsafe {
        ...
        match read_file(file_handle, &mut file_buffer) {
            Ok(number_of_bytes_read) => {
                close_file(file_handle);
